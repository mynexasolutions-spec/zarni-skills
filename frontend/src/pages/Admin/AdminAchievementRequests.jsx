...
import { Award, CheckCircle, XCircle, Clock, Plane, Trophy, Check, X, Sparkles } from 'lucide-react';
...
function Avatar({ name }) {
  ...
  return (
    <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-purple-500 via-indigo-600 to-pink-500 text-white text-xs font-black flex items-center justify-center shrink-0 shadow-sm">
      {initials}
    </div>
  );
}

export default function AdminAchievementRequests() {
  ...
  const fetchRequests = async () => {
    try {
      const res = await api.get('/admin/achievement-requests');
  ...
  const updateStatus = async (id, status, promptNote) => {
    ...
    setBusyId(id);
    try {
      await api.post(`/admin/achievement-requests/${id}/status`, { status, admin_note });
      fetchRequests();
    } catch (err) {
      console.error(err);
    } finally {
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-400 text-xs font-semibold uppercase tracking-widest animate-pulse">Loading Achievement Requests...</p>
      </div>
    );
  }
    ...
    <div className="text-slate-800 space-y-6 pb-10 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="relative shrink-0">
            <div className="absolute -inset-1 rounded-2xl bg-red-500/20 blur-md animate-pulse"></div>
            <div className="relative w-12 h-12 rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 text-white flex items-center justify-center shadow-lg shadow-red-500/20">
              <Award className="w-5.5 h-5.5" />
            </div>
          </div>
          <div>
            <h2 className="text-2xl font-black tracking-tight text-slate-900 leading-tight">Achievement & Badge Claims</h2>
            <p className="text-xs text-slate-400 font-semibold">Verify student milestone reward badges and Trip Achievement requests</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { key: 'approved', label: 'Approved Claims', Icon: CheckCircle, color: 'from-emerald-500 to-teal-600', shadow: 'shadow-emerald-500/15', text: 'text-emerald-100' },
          { key: 'rejected', label: 'Rejected Claims', Icon: XCircle, color: 'from-rose-600 to-red-700', shadow: 'shadow-red-600/15', text: 'text-rose-100' },
        ].map((kpi, idx) => (
          <div key={kpi.key} className={`group relative overflow-hidden rounded-[1.8rem] p-5 text-white bg-gradient-to-br ${kpi.color} shadow-lg ${kpi.shadow} transition-all duration-300 hover:-translate-y-0.5 animate-fade-in-up`} style={{ animationDelay: `${idx * 70}ms` }}>
            <kpi.Icon className="absolute -right-3 -bottom-3 w-16 h-16 text-white/10 pointer-events-none transition-transform duration-500 group-hover:scale-110" strokeWidth={1.5} />
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar p-1.5 bg-slate-100/70 border border-slate-200/60 rounded-2xl">
        {STATUS_TABS.map(tab => (
          <button
            ...
            className={`shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all duration-200 ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filtered.map((r, idx) => {
          const isTrip = r.achievement_title === 'Trip Achievement';
          const TypeIcon = isTrip ? Plane : Trophy;
          return (
            <div key={r.id} className="group bg-white border border-slate-200/80 rounded-[2rem] p-6 shadow-sm hover:shadow-xl hover:shadow-slate-100 hover:-translate-y-1 transition-all duration-300 animate-fade-in-up flex flex-col justify-between" style={{ animationDelay: `${Math.min(idx, 8) * 60}ms` }}>
              <div className="space-y-4">
                ...
                {/* Header user & badge info */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3.5 min-w-0">
                    <Avatar name={r.user_name} />
                    <div className="min-w-0">
                      <p className="font-extrabold text-slate-900 text-base leading-snug group-hover:text-red-600 transition-colors truncate">{r.user_name || 'Student'}</p>
                      <div className="mt-1 flex items-center gap-1.5">
                        <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 rounded-full border ${
                          isTrip ? 'bg-sky-50 text-sky-600 border-sky-200/40' : 'bg-indigo-50 text-indigo-600 border-indigo-200/40'
                        }`}>
                          <TypeIcon className="w-3.5 h-3.5" /> {r.achievement_title}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* Claim note container */}
                <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-4 space-y-2 text-xs">
                  {r.note ? (
                    <p className="text-slate-700 leading-relaxed font-medium">"{r.note}"</p>
                  ) : (
                    <p className="text-slate-400 italic font-medium">No note attached to this claim.</p>
                  )}
        ...
        {filtered.length === 0 && (
          <div className="col-span-full text-center py-20 bg-white rounded-3xl border border-slate-200/80 text-slate-400 font-semibold shadow-sm">
            <Award className="w-12 h-12 mx-auto text-slate-200 mb-3 animate-pulse" />
            No achievement claims found in the "{statusFilter}" category.
          </div>
        )}
      </div>
