...
export default function ManagerCommissions() {
  ...
  const [totals, setTotals] = useState({ total_earnings: 0, available_balance: 0, pending_earnings: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchCommissions = async () => {
      try {
        const response = await api.get('/manager/commissions');
        ...
        console.error('Error fetching manager commissions', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const kpis = [
    { label: 'Total Earned', value: totals.total_earnings, icon: CheckCircle2, color: 'from-emerald-500 to-green-600' },
    { label: 'Available Balance', value: totals.available_balance, icon: Wallet, color: 'from-violet-500 to-purple-600' },
    ...
    <div className="text-slate-800">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium">{commissions.length} commission{commissions.length !== 1 ? 's' : ''} recorded</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm text-slate-500">
            ...
            <tbody>
                ...
                <tr key={c.id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3.5">
                    <span className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-full ${
                      c.status === 'approved' ? 'bg-emerald-50 text-emerald-700' : c.status === 'paid' ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700'
                    }`}>{c.status}</span>
                  </td>
                </tr>
              ))}
              {commissions.length === 0 && (
                <tr>
                  <td colSpan="6" className="text-center py-12 text-slate-400 font-medium">No commissions yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="lg:hidden space-y-3">
          ...
          <div key={c.id} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-400">{c.created_at} · Sale ₹{c.sale_amount.toLocaleString('en-IN')} · {c.commission_percent}%</p>
              </div>
        ...
        {commissions.length === 0 && (
          <div className="text-center py-10 bg-white rounded-2xl border border-slate-100 text-slate-400 font-medium">No commissions yet.</div>
        )}
      </div>
    </div>
  );
