...
export default function AdminSidebar({ onNavigate }) {
  ...
  return (
    <aside className="w-[280px] min-w-[280px] flex flex-col h-full border-r border-slate-200 shadow-[2px_0_8px_rgba(0,0,0,0.04)]" style={{ background: 'linear-gradient(180deg, #fff 0%, #fafafa 100%)' }}>
      ...
      <div className="relative flex-shrink-0 overflow-hidden" style={{ background: 'linear-gradient(135deg, #1c0b14 0%, #7f1d1d 60%, #dc2626 100%)' }}>
        <div className="absolute -top-8 -right-8 w-28 h-28 bg-red-500/25 rounded-full blur-2xl pointer-events-none"></div>
        <div className="relative flex items-center gap-3 px-4 py-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[0.7rem] text-red-200 font-bold uppercase tracking-wider">Administrator</p>
          </div>
        </div>
      </div>
                ...
                return (
                  <li key={iIdx} className="mx-2">
                    <Link
                      ...
                      style={isActive ? { background: 'linear-gradient(135deg, #fef2f2 0%, #fff1f2 100%)', boxShadow: 'inset 0 0 0 1px rgba(220,38,38,0.12)' } : undefined}
                    >
                      <span
                        className={`absolute left-[-8px] top-1/2 -translate-y-1/2 w-[3px] rounded-r transition-all duration-200 ${isActive ? 'h-5 bg-red-600' : 'h-0 bg-red-600 group-hover:h-3.5'}`}
                      ></span>
                      <span
                        className="flex items-center justify-center rounded-[9px] p-[7px] flex-shrink-0 transition-all duration-200"
                        style={isActive
                          ? { background: 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)', color: '#fff', boxShadow: '0 4px 10px rgba(220,38,38,0.35)' }
      ...
      <div className="flex-shrink-0 p-3 border-t border-slate-100 bg-slate-50 flex flex-col gap-2">
        <Link
          to="/"
          className="flex items-center justify-center gap-2 py-2.5 rounded-[10px] text-white font-bold text-[0.85rem] shadow-[0_4px_12px_rgba(220,38,38,0.3)] hover:shadow-[0_6px_16px_rgba(220,38,38,0.4)] hover:-translate-y-0.5 transition-all"
